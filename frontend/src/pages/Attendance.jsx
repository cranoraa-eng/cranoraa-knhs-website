...
const Attendance = () => {
  ...
  const selectSchedule = async (sch) => {
    ...
    try {
      const res = await api.get(
        `/attendance/by_schedule/?schedule=${sch.id}&date=${todayStr}`,
      ...
      students.forEach(s => {
        if (s.status) {
          ...
          saved[s.student_id] = { id: s.attendance_id, status: s.status, remarks: s.remarks || '' };
  ...
  const scheduleHasChanges = useMemo(() => {
    return scheduleStudents.some(s => {
      const draftStatus = scheduleDraft[s.student_id]?.status;
      const savedStatus = scheduleSaved[s.student_id]?.status;
      const draftRemarks = scheduleRemarks[s.student_id] || '';
      const savedRemarks = scheduleSaved[s.student_id]?.remarks || '';
      return draftStatus !== savedStatus || draftRemarks !== savedRemarks;
    });
  }, [scheduleStudents, scheduleDraft, scheduleSaved, scheduleRemarks]);

  const scheduleStats = useMemo(() => {
    return scheduleStudents.reduce((acc, s) => {
  ...
  }, [scheduleStudents, scheduleDraft]);
                          ...
                          <Card>
                            <CardBody className="p-0">
                              ...
                              <button key={key} onClick={() => markScheduleDraft(s.student_id, key)}
                                                        ...
                                                        aria-label={`Mark ${s.student_name} as ${cfg.label}`}
