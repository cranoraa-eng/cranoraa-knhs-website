                  ...
                  <td className="px-3 py-3 text-center relative">
                    <div className="hidden md:flex items-center justify-center gap-1">
                      ...
                      {(app.status === 'pending' || app.status === 'under_review') && (
                        <>
                          <button onClick={() => promptApprove(app.id)} className="p-1.5 text-slate-400 hover:text-emerald-600 hover:bg-emerald-50 rounded-lg" title="Approve">
                    ...
                    <div className="md:hidden">
                      ...
                      {activeMenu === app.id && (
                        <div className="absolute right-0 top-full mt-1 bg-white border border-slate-100 rounded-xl shadow-xl z-50 py-1 min-w-[130px]">
                          ...
                          {(app.status === 'pending' || app.status === 'under_review') && (
                            <>
                              <button onClick={() => { promptApprove(app.id); setActiveMenu(null); }} className="w-full text-left px-3 py-2 text-[10px] font-bold text-emerald-600 hover:bg-emerald-50 flex items-center gap-2">Approve</button>
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-6">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-3 text-sm">
                  <div className="bg-emerald-50 p-3 rounded-xl"><p className="text-[9px] font-bold text-emerald-600 uppercase">Father</p><p className="font-bold text-slate-800">{selected.father_name}</p><p className="text-xs text-slate-500">{selected.father_contact}</p></div>
                  <div className="bg-rose-50 p-3 rounded-xl"><p className="text-[9px] font-bold text-rose-600 uppercase">Mother</p><p className="font-bold text-slate-800">{selected.mother_name}</p><p className="text-xs text-slate-500">{selected.mother_contact}</p></div>
              ...
              <div className="flex gap-2">
                {(selected.status === 'pending' || selected.status === 'under_review') && (
                  <>
                    ...
                    <button onClick={() => { promptApprove(selected.id); }} className="px-4 py-2 rounded-lg bg-emerald-600 text-white text-xs font-bold hover:bg-emerald-700">Approve</button>
                  </>
                )}
