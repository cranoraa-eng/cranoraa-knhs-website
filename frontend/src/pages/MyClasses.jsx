          ...
          {groupedAssignments.map((classroom) => (
            <div key={classroom.id} className="bg-white border border-gray-200 rounded-2xl p-5 md:p-6 hover:shadow-xl transition-all duration-300 group relative border-t-4 border-t-indigo-600 overflow-hidden flex flex-col h-full">
              ...
              <div className="space-y-2 md:space-y-3 mb-6">
                ...
                <div className="border border-slate-100 rounded-xl overflow-hidden shadow-sm bg-white h-[140px] md:h-[160px] overflow-y-auto scrollbar-thin scrollbar-thumb-slate-100">
              ...
              <div className="flex-1 space-y-2 md:space-y-3">
                ...
                <div className="border border-slate-100 rounded-xl overflow-hidden shadow-sm bg-white h-[200px] md:h-[240px] overflow-y-auto scrollbar-thin scrollbar-thumb-slate-100">
                  <table className="w-full text-left table-fixed border-separate border-spacing-0">
                    <thead className="bg-slate-50 border-b border-slate-100 sticky top-0 z-10">
                      <tr>
                        <th className="px-3 py-2 text-[8px] md:text-[10px] font-black text-slate-400 uppercase tracking-widest w-1/2">Student Name</th>
                        <th className="px-3 py-2 text-[8px] md:text-[10px] font-black text-slate-400 uppercase tracking-widest">LRN/ID</th>
                        <th className="px-3 py-2 text-[8px] md:text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">Actions</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-50">
                      {classroom.students.length === 0 ? (
                        <tr>
                          <td colSpan="3" className="px-3 py-6 text-center text-[10px] font-bold text-slate-300 uppercase tracking-widest italic">No students enrolled</td>
                        </tr>
                      ) : (
                          ...
                          <tr key={student.id} className="hover:bg-slate-50/50 transition-colors">
                            <td className="px-3 py-2">
                              <div className="flex items-center gap-2">
                                <span className="text-[8px] md:text-[10px] font-black text-slate-300 w-3">{idx + 1}</span>
                                <span className="text-[9px] md:text-xs font-bold text-slate-700 uppercase">{student.last_name}, {student.first_name}</span>
                              </div>
                            </td>
                            <td className="px-3 py-2 text-[8px] md:text-[10px] font-mono font-bold text-slate-400 uppercase">{student.profile?.registration_number || student.username}</td>
                            <td className="px-3 py-2">
            ...
            <div className="p-8">
              <div className="grid grid-cols-2 gap-x-12 gap-y-6">
                ...
                <div className="space-y-4">
                  ...
                  <ProfileField label="Classroom" value={selectedStudent.profile?.classroom_name} />
                  <ProfileField label="Gender" value={selectedStudent.profile?.sex} />
                </div>
              </div>
