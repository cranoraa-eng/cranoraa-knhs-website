      ...
      <SectionCard title="School Identity" subtitle="Displayed across the portal and public website" icon="🏫">
        ...
        <Field label="School Address">
          <textarea value={form.school_address} onChange={e => setForm(p => ({...p, school_address: e.target.value}))}
            ...
            className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm font-medium text-slate-800 focus:outline-none focus:ring-2 focus:ring-violet-500/20 focus:border-violet-400 focus:bg-white transition-all resize-none" />
        </Field>
      </SectionCard>
          ...
          <div className="space-y-2">
            {years.map(y => (
              <div key={y.id} className={`flex items-center justify-between p-4 rounded-xl border transition-all ${y.is_active ? 'bg-violet-50 border-violet-200' : 'bg-slate-50 border-slate-100 hover:bg-white hover:border-slate-200'}`}>
                <div className="flex items-center gap-3">
                  <div className={`w-2.5 h-2.5 rounded-full flex-shrink-0 ${y.is_active ? 'bg-violet-500 shadow-[0_0_6px_rgba(139,92,246,0.6)]' : 'bg-slate-300'}`} />
                  <div>
                    <p className={`text-sm font-black ${y.is_active ? 'text-violet-800' : 'text-slate-800'}`}>{y.name}</p>
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                      {y.start_date} → {y.end_date}
                      {y.is_active && <span className="ml-2 text-violet-500">● Active</span>}
                    </p>
                  </div>
                </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <Field label="Current Quarter" hint="Used as default when entering grades">
            <select value={settings.current_quarter} onChange={e => setSettings(p => ({...p, current_quarter: e.target.value}))}
              className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold text-slate-800 focus:outline-none focus:ring-2 focus:ring-violet-500/20 focus:border-violet-400 transition-all">
      ...
      <SectionCard title="Profile Information" subtitle="Your personal details visible to the school" icon="👤">
        ...
        <div className="flex items-center gap-5 mb-6">
          <div className="relative">
            <div className="w-20 h-20 rounded-2xl bg-gradient-to-br from-violet-500 to-indigo-600 flex items-center justify-center text-white text-2xl font-black overflow-hidden shadow-lg">
