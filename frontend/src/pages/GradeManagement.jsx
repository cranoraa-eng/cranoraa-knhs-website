...
const ScoreBadge = ({ score }) => {
  ...
  const cls = n >= 90 ? 'text-green-700 bg-green-50 border-green-200'
    : n >= 85 ? 'text-blue-700 bg-blue-50 border-blue-200'
    : n >= 80 ? 'text-yellow-700 bg-yellow-50 border-yellow-200'
    : n >= 75 ? 'text-orange-700 bg-orange-50 border-orange-200'
    : 'text-red-700 bg-red-50 border-red-200';
  return <span className={`inline-flex items-center justify-center min-w-[2.5rem] px-2 py-0.5 rounded-lg border text-sm font-bold ${cls}`}>{n}</span>;
};
    ...
    <div className="animate-in fade-in duration-700">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6 animate-fade-in-up">
        <div className="text-center md:text-left">
          <h1 className="text-2xl md:text-3xl font-bold text-gray-800">Grade Management</h1>
          <p className="text-sm md:text-base text-gray-500 mt-1">Final grades organized horizontally by Student (Q1–Q4)</p>
        </div>
        {(user?.role === 'admin' || user?.role === 'teacher') && (
          <button onClick={() => navigate('/grade-input')}
            className="flex items-center justify-center gap-2 bg-purple-600 hover:bg-purple-700 text-white font-medium py-2.5 px-6 rounded-xl transition-all hover:scale-105 active:scale-95 shadow-sm w-full md:w-auto">
      ...
      {/* Filters */}
      <div className="bg-white border border-gray-200 rounded-xl shadow-sm p-4 mb-5 animate-fade-in-up [animation-delay:100ms]">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          <div className="relative group w-full">
            ...
            <input type="text" placeholder="Search student or subject..."
              ...
              className="w-full pl-9 pr-4 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 text-sm transition-all shadow-sm" />
          </div>
          <div className="relative w-full">
            <select value={filterQuarter} onChange={e => setFilterQuarter(e.target.value)}
              className="w-full px-3 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 text-sm transition-all hover:border-purple-300 hover:shadow-sm cursor-pointer appearance-none bg-[url('data:image/svg+xml;charset=US-ASCII,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20width%3D%2224%22%20height%3D%2224%22%20viewBox%3D%220%200%2024%2024%22%20fill%3D%22none%22%20stroke%3D%22%236B7280%22%20stroke-width%3D%222%22%20stroke-linecap%3D%22round%22%20stroke-linejoin%3D%22round%22%3E%3Cpolyline%20points%3D%226%209%2012%2015%2018%209%22%3E%3C%2Fpolyline%3E%3C%2Fsvg%3E')] bg-[length:1.25rem] bg-[right_0.5rem_center] bg-no-repeat pr-10 shadow-sm">
          ...
          <div className="flex items-center border border-gray-300 rounded-lg overflow-hidden bg-white transition-all focus-within:ring-2 focus-within:ring-purple-500 shadow-sm w-full">
            <button 
              onClick={() => handleYearChange('prev')}
              className="px-4 py-2.5 hover:bg-gray-50 text-gray-500 border-r border-gray-200 transition-colors active:bg-gray-100 flex-shrink-0"
            >
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" />
              </svg>
            </button>
            <div className="flex-1 text-center text-sm font-semibold text-gray-700 select-none min-w-[80px]">
              {filterYear}
            </div>
            <button 
              onClick={() => handleYearChange('next')}
              className="px-4 py-2.5 hover:bg-gray-50 text-gray-500 border-l border-gray-200 transition-colors active:bg-gray-100 flex-shrink-0"
            >
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
              </svg>
      ...
      {loading ? (
        ...
        <div className="bg-white border border-gray-200 rounded-xl shadow-sm p-16 text-center text-gray-400 animate-fade-in-up [animation-delay:200ms]">
          No final grades recorded yet. Use "Input Grades" to start entering grades.
        </div>
      ) : (
            ...
            return (
              <div key={level} className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden transition-all hover:shadow-md">
                ...
                <button onClick={() => setOpenLevel(levelOpen ? null : level)}
                  className="w-full flex items-center justify-between px-6 py-4 bg-gradient-to-r from-[#2D1B4D] to-[#4B2D7F] text-white hover:from-[#3D2B5D] hover:to-[#5B3D8F] transition-all group">
                  <div className="flex items-center gap-3 transition-transform group-hover:translate-x-1">
                    ...
                    <div className="text-left">
                      ...
                      <div className="text-purple-200 text-xs">{levelClassrooms.length} classroom{levelClassrooms.length !== 1 ? 's' : ''} · {levelCount} grade entries</div>
                    </div>
                  </div>
                      ...
                      return (
                        <div key={classroom.id}>
                          ...
                          <button onClick={() => setOpenClassroom(classroomOpen ? null : classroom.id)}
                            className="w-full flex items-center justify-between px-6 py-3.5 bg-purple-50 hover:bg-purple-100 transition-colors group">
                            <div className="flex items-center gap-3 transition-transform group-hover:translate-x-1">
                              ...
                              <div className="text-left">
                                <div className="font-semibold text-gray-800">{classroom.name}</div>
                                <div className="text-xs text-gray-500">{subjects.length} subject{subjects.length !== 1 ? 's' : ''} · {classroomGrades.length} entries · Adviser: {classroom.teacher_name || 'N/A'}</div>
                              </div>
                            </div>
                                ...
                                return (
                                  <div key={subject.id} className="border-t border-gray-100">
                                    ...
                                    <button onClick={() => setOpenSubject(subjectOpen ? null : subjectKey)}
                                      className="w-full flex items-center justify-between px-8 py-3 bg-gray-50 hover:bg-indigo-50 transition-colors group">
                                      <div className="flex items-center gap-3 transition-transform group-hover:translate-x-1">
                                        <span className="font-mono text-xs font-bold text-purple-700 bg-purple-100 px-2 py-1 rounded">
                                          {subject.code}
                                        </span>
                                        <div className="text-left">
                                          <div className="font-semibold text-gray-800 text-sm">{subject.name}</div>
                                          <div className="text-xs text-gray-400">{studentsList.length} student{studentsList.length !== 1 ? 's' : ''}</div>
                                        </div>
                                      </div>
                                    ...
                                    {subjectOpen && (
                                      <div className="px-4 md:px-8 pb-4 pt-2 animate-in fade-in slide-in-from-top-2 duration-300">
                                        <div className="overflow-x-auto rounded-xl border border-gray-200 shadow-sm scrollbar-thin scrollbar-thumb-gray-300">
                                          <table className="w-full text-sm">
                                            <thead className="bg-gray-50 border-b border-gray-200">
                                              <tr>
                                                <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider sticky left-0 bg-gray-50 z-10 w-12 border-r border-gray-100">#</th>
                                                <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider sticky left-12 bg-gray-50 z-10 border-r border-gray-200">Student</th>
                                                <th className="text-center px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider bg-blue-50/30">Q1</th>
                                                <th className="text-center px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider bg-blue-50/30">Q2</th>
                                                <th className="text-center px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider bg-blue-50/30">Q3</th>
                                                <th className="text-center px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider bg-blue-50/30">Q4</th>
                                                <th className="text-center px-4 py-3 text-xs font-semibold text-purple-600 uppercase tracking-wider border-l border-gray-200 bg-purple-50/30">Final</th>
                                                <th className="text-center px-4 py-3 text-xs font-semibold text-indigo-600 uppercase tracking-wider bg-indigo-50/30">Rounded</th>
                                                <th className="text-center px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider bg-purple-50/30">Remarks</th>
                                                {(user?.role === 'admin' || user?.role === 'teacher') && (
                                              ...
                                              {studentsList.map((s, idx) => {
                                                ...
                                                return (
                                                  <tr key={s.id} className="hover:bg-purple-50 transition-colors group/row text-center">
                                                    <td className="px-4 py-3 text-gray-400 text-xs sticky left-0 bg-white group-hover/row:bg-purple-50 z-10 w-12 border-r border-gray-100">{idx + 1}</td>
                                                    <td className="px-4 py-3 sticky left-12 bg-white group-hover/row:bg-purple-50 z-10 border-r border-gray-200">
                                                      <div className="flex items-center gap-2 min-w-[180px] text-left">
                                                        <div className="w-8 h-8 rounded-full bg-gradient-to-br from-purple-500 to-indigo-500 flex items-center justify-center text-white font-bold text-xs flex-shrink-0 shadow-sm">
                                                          {s.name?.charAt(0).toUpperCase()}
                                                        </div>
                                                        <div>
                                                          <div className="font-semibold text-gray-800 leading-tight whitespace-nowrap">{s.name}</div>
                                                          <div className="text-[10px] text-gray-400 truncate max-w-[120px]">{s.email}</div>
                                                        </div>
                                                      </div>
                                                    </td>
                                                    {[1, 2, 3, 4].map(qNum => (
                                                      <td key={qNum} className="px-2 py-3 text-center border-l border-gray-50">
                                                        <div className="flex flex-col items-center gap-1">
                                                          <ScoreBadge score={s.quarters[qNum]?.raw_score} />
                                                          {s.quarters[qNum]?.is_locked && <span className="text-[10px]" title="Locked">🔒</span>}
                                                        </div>
                                                      </td>
                                                    ))}
                                                    <td className="px-4 py-3 text-center border-l border-gray-200 bg-purple-50/10">
                                                    ...
                                                    <td className="px-4 py-3 text-center bg-purple-50/10">
                                                      {remarks ? (
                                                        <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full border whitespace-nowrap transition-all group-hover/row:scale-105 ${REMARKS_STYLE[remarks]}`}>
                                                          {remarks}
                                                        </span>
                                                      ) : '—'}
                                                    </td>
                                                    {(user?.role === 'admin' || user?.role === 'teacher') && (
                                                      <td className="px-4 py-3 text-center">
                                                        <div className="flex items-center justify-center gap-1 md:opacity-0 group-hover/row:opacity-100 transition-opacity">
      ...
      {!loading && (
        <p className="text-sm text-gray-400 mt-4 animate-fade-in-up [animation-delay:400ms]">
          {filteredGrades.length} grade entries grouped by student across {sortedLevels.length} grade level{sortedLevels.length !== 1 ? 's' : ''}
        </p>
      )}
    </div>
  );
